...
func (t *TeamService) AddTeam(ctx context.Context, team *model.Team) *Error {
	err := t.tx.WithinTransaction(ctx, func(txCtx context.Context) error {
		...
		if errors.Is(err, repository.ErrAlreadyExists) {
			return NewServiceError(ErrorCodeTeamExists, "team_name already exists")
		}
		if err != nil {
			return NewServiceError(ErrorCodeUnspecified, "failed to create team")
		}
		...
		for _, user := range team.Members {
			if err = t.users.Upsert(txCtx, &repository.User{
				...
				return NewServiceError(ErrorCodeUnspecified, "failed to upsert team member")
			}
		}
...
func (t *TeamService) GetTeam(ctx context.Context, name string) (*model.Team, *Error) {
	// TODO: Надо подумать, мб запихать это в один вызов GetTeamMembers и все
	teamRepo, err := t.teams.Get(ctx, name)
	if errors.Is(err, repository.ErrNotFound) {
		return nil, NewServiceError(ErrorCodeNotFound, "team not found")
	}
	if err != nil {
		return nil, NewServiceError(ErrorCodeUnspecified, "failed to get team")
	}

	membersRepo, err := t.teams.GetTeamMembers(ctx, name)
	if err != nil {
		return nil, NewServiceError(ErrorCodeUnspecified, "failed to get team members")
	}
